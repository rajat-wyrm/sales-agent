"""
Tier 2: AngelList/Wellfound startup job listings scraper.

Wellfound (formerly AngelList) has a job search page that uses JS rendering.
Uses Playwright browser automation (SRS §3.3).
...
class WellfoundScraper(BaseScraper):
    ...
    API_URL = "https://wellfound.com/jobs?fresher=true"
    ...
    async def _scrape_with_playwright(self, url: str) -> str:
        try:
            ...
            raise ScraperError("Playwright not installed — required for Wellfound (JS-heavy)")
    ...
    async def scrape(self) -> list[dict[str, Any]]:
        ...
        if not BS4_AVAILABLE:
            raise ScraperError("beautifulsoup4 required for Wellfound parsing")

        soup = BeautifulSoup(html, "html.parser")

        job_cards = soup.find_all("div", class_=re.compile(r"jobCard|job-card|cardContainer"))
        if not job_cards:
        ...
        for card in job_cards[:100]:
            ...
            company_elems = card.find_all(attrs={"class": re.compile(r"company|startup|org")})
            ...
            location_elems = card.find_all(attrs={"class": re.compile(r"location|loc|remote")})
        ...
        self._logger.info(f"Wellfound: scraped {len(leads)} raw leads")
        return leads
