"""
Tier 2: Instahyre job listings scraper.

Uses Playwright browser automation (SRS §3.3) — Instahyre is JS-rendered.
...
class InstahyreScraper(BaseScraper):
    ...
    API_URL = "https://www.instahyre.com/fresher-jobs"
    ...
    async def _scrape_with_playwright(self, url: str) -> str:
        try:
            ...
            raise ScraperError("Playwright not installed — required for Instahyre (JS-heavy)")
    ...
    async def scrape(self) -> list[dict[str, Any]]:
        ...
        if not BS4_AVAILABLE:
            raise ScraperError("beautifulsoup4 required for Instahyre parsing")

        soup = BeautifulSoup(html, "html.parser")

        job_cards = soup.find_all("div", class_=re.compile(r"jobCard|job-card|c-job"))
        if not job_cards:
            job_cards = soup.find_all("div", attrs={"data-job-id": True})

        for card in job_cards[:100]:
            title_elems = card.find_all(attrs={"class": re.compile(r"title|jobTitle|role|position")})
        ...
        self._logger.info(f"Instahyre: scraped {len(leads)} raw leads")
        return leads
