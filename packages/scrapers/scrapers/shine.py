"""
Tier 2: Shine.com job listings scraper.

Shine.com is a major Indian job portal. Uses Playwright browser automation (SRS §3.3).
...
class ShineScraper(BaseScraper):
    ...
    API_URL = "https://www.shine.com/job-search/fresher-jobs"
    ...
    async def _scrape_with_playwright(self, url: str) -> str:
        try:
            ...
            raise ScraperError("Playwright not installed — required for Shine (JS-heavy)")
    ...
    async def scrape(self) -> list[dict[str, Any]]:
        ...
        if not BS4_AVAILABLE:
            raise ScraperError("beautifulsoup4 required for Shine parsing")

        soup = BeautifulSoup(html, "html.parser")

        job_cards = soup.find_all("div", class_=re.compile(r"jobCard|job-card|searchList"))
        if not job_cards:
        ...
        for card in job_cards[:100]:
            ...
            company_elems = card.find_all(attrs={"class": re.compile(r"company|organization|emp")})
        ...
        self._logger.info(f"Shine: scraped {len(leads)} raw leads")
        return leads
